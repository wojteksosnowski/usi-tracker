...
10. **Investment Merge Schema Consistency**: T3 Master files (`inv_master_*.json`) strictly group records using the keys `"primary_id"` (to denote the main record) and `"members"` (a list of all aggregated investments). All repository readers (e.g. `InvestmentRepository.get_master_data`) MUST use these exact keys. Legacy keys such as `"master_usi_inv_id"` or `"merged_from"` are strictly deprecated for T3 investment merges and will cause silent aggregation failures if used.
...
14. **Primary ID jako kotwica indeksu**: Plik `inv_master_*.json` (i tylko on) przechowuje klucz `"primary_id"`. Jest on wykorzystywany przez mechanizmy (np. `InvestmentIndex`) wyłącznie po to, by w locie wyświetlić główny rekord i ukryć pozostałe przypisane do tego samego `master_id`.
