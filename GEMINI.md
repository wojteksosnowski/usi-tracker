...
- **API First for Raw Data Parsing**: NEVER write ad-hoc `json.load()` scripts to manually extract information from `raw_*.json` files. Always check `usi_scrapers.api` and `usi_scrapers.mapping` (e.g., `transform_to_unified()`) to leverage the library's official transformation mappings.
- **Library Reinstallation after Modifications**: `usi-tracker` consumes `usi-scrapers` as an installed package. Any time you modify source files inside the `usi-scrapers` directory (e.g., `transformers.py` or `portal_data_mapping.json`), you MUST run `venv/bin/pip install -e /path/to/usi-scrapers` inside the tracker before running local tests or scripts, otherwise your changes will be ignored.
...
8.  **Strict Investment Sources Block**: Słownik `sources` wewnątrz docelowych plików inwestycji (`usi_{inv_slug}.json`) MUSI rygorystycznie opierać się wyłącznie na kluczu `"id"` (np. `sources.oto.id`). Kategorycznie ZABRANIA SIĘ wykorzystywania generatorów mapowania deweloperskiego (np. `ScraperGateway.generate_portal_mapping`) do uzupełniania inwestycji. Doprowadza to do wstrzyknięcia kluczy typu `agency_id`, co trwale psuje lokalny system detekcji dubli (`DeveloperIndexer`) i skutkuje nieskończonym powielaniem posiadanych danych przy każdym skanowaniu portalu.
9.  **Data Source URL Resolution**: Due to the Strict Investment Sources Block mandate, the `sources` mapping inside `usi_*.json` is designed strictly to hold portal `id`s. UI elements and backend loaders (e.g., `_resolve_source_data`) MUST NOT assume that a `"url"` key will always be nested within `sources.{portal}` (this is especially true for older records like Otodom/TabelaOfert). If a URL is missing from `sources`, the system MUST employ a robust fallback mechanism to extract it from the top-level `"website"` key or dynamically generate it.
10. **Investment Merge Schema Consistency**: T3 Master files (`inv_master_*.json`) strictly group records using ONLY the key `"members"` (a list of all aggregated investments). All repository readers (e.g. `InvestmentRepository.get_master_data`) MUST use this exact key. Legacy keys such as `"primary_id"`, `"master_usi_inv_id"`, or `"merged_from"` are strictly deprecated for T3 investment merges and will cause silent aggregation failures if used.
11. **Batch Pipeline & Image Synchronization**: The `usi-scrapers` library handles physical file downloads during batch ingest operations (e.g., `process_batch_ingest`), but it DOES NOT return localized `image_paths` directly in the payload. Any batch processing loop within `usi-tracker` (like `process_batch`) MUST explicitly invoke the `ImageSyncService` (or equivalent local resolver) to scan the target directory and populate the `image_paths` array in the unified JSON. Never assume that `transform_to_unified` will provide `image_paths` (it only provides remote `image_urls`). Bypassing this step leaves investments without image links in the UI.
12. **Anti-Virtualization Rule (Prymat Dysku)**: Pod żadnym pozorem nie generuj "wirtualnych" (trzymanych wyłącznie w pamięci RAM) instancji inwestycji ani grup w plikach takich jak `investment_index.py`. Każda grupa i każdy rekord musi mieć fizyczne, zgodne z `CANONICAL.md` odzwierciedlenie na dysku.
13. **Strict Flat JSON Rule dla Grup**: Złączone inwestycje (poziom T3) oznaczane są w plikach cząstkowych (`usi_*.json`) wyłącznie JEDNYM kluczem: `"master_id"`. Bezwzględnie zabrania się wstrzykiwania do plików `usi_*.json` flag typu `is_secondary`, `is_grouped` czy `master_primary_id`.
14. **Brak głównego rekordu (No Hierarchy)**: Plik `inv_master_*.json` jest wyłącznie płaską listą równoprawnych członków i pod żadnym pozorem NIE przechowuje klucza `"primary_id"`. Mechanizmy (np. `InvestmentIndex`) używają po prostu pierwszego załączonego elementu jako kotwicy wyświetlania, by ukryć pozostałe przypisane do tego samego `master_id`.
