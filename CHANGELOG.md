...
### Zmieniono
- **SRP w `DeveloperRepository`**: Usunięto antywzorzec wstrzykiwania `DeveloperService` oraz skanowania katalogów (`unregistered_count`) z głębinowej metody `_enrich_with_master`. Logika ta, powodująca I/O thrashing na liście deweloperów, została wyodrębniona i przypięta poprawnie w warstwie biznesowej do `DeveloperService.get_developer_enriched`.
...
- **Wydajność operacji CLI**: Do `main.py` wprowadzono pomocniczą metodę `_resolve_system_id_from_slugs`, która eliminuje bezpośrednie czytanie liniowe plików `_index.json`. Skrypty (jak `update-inv` czy `download-raw`) pobierają teraz ID natychmiast z gorącego indeksu w pamięci RAM poprzez `get_investment_index()`.
- **Ochrona ID-only w skryptach systemowych**: Skrypt konserwacyjny `backfill_usi_ids` w `main.py` został przepisany. Zamiast brutalnego iterowania po systemie plików (`rglob`), używa teraz `InvestmentRepository.get_all_system_ids()` oraz oficjalnych funkcji zapisujących w repozytoriach. Zapobiega to omijaniu architektonicznych resolwerów ścieżek.
- **Bezpieczeństwo testów CI**: Dodano plik `tests/test_environment_sanity.py`, służący jako test bezpiecznikowy chroniący proces CI przed oszukiwaniem (cichym pomijaniem weryfikacji braku środowiskowych plików referencyjnych przez wywołania `skipif`).
